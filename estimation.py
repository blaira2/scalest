...
class Estimator:
    ...
    def estimate_heights(self, building_mask,
        ...
        for frac in fractions:
            ...
            if bottom is None:
                # projected ground intersection is off-screen (or behind camera)
                continue

            u_g, v_g = bottom
            u_bottom = int(round(u0))   # keep same column as the building line
            v_bottom = int(round(v_g))
    ...
    def fit_plane_ransac(self, points, num_iterations=1000, min_sample_dist=0.1, distance_thresh=0.05):
    ...
    def estimate_ground_plane(self, leftIm, rightIm,
        ...
        stereo = cv.StereoSGBM_create(minDisparity=0, numDisparities=128, blockSize=7)
        ...
        pts3d_img = cv.reprojectImageTo3D(dispL, Q)
        
        #get a subset of the dense points
        stride = 6
        valid = (ground_mask > 0) & (dispL > 1.0)
        valid_sub = np.zeros_like(valid, dtype=bool)
        valid_sub[::stride, ::stride] = valid[::stride, ::stride]
        
        #L-R, R-L disparity check
        tau = 2.5  
        ys, xs = np.where(valid_sub) 
        dL = dispL[ys, xs]
        xr = xs - dL 

        # in-bounds in right image
        inbounds = (xr >= 0) & (xr < (width - 1))
        xri = np.clip(np.round(xr).astype(np.int32), 0, width - 1)
        dR = dispR[ys, xri]

        # keep if disparities agree
        lr_ok = np.zeros_like(valid_sub, dtype=bool)
        lr_ok[ys, xs] = inbounds & (dL > 0) & (dR > 0) & (np.abs(dL - dR) <= tau)
        
        valid_sub &= lr_ok
        
        
        pts3d_gr_valid = pts3d_img[valid_sub]
        pts2d_gr_valid = np.column_stack(np.where(valid_sub)[::-1]).astype(np.float64)


        n_ground, d_ground, inliers_ground = self.fit_plane_ransac(
            pts3d_gr_valid, distance_thresh=ransac_thresh
        )

        return n_ground, d_ground, inliers_ground, pts2d_gr_valid, pts3d_gr_valid
    ...
    def visualize_ground_plane_on_image(self,
        ...
        p0 = None  # find a visible point
        for X in P_inliers:
            uv = self.project_point(X, focal_length_pixels, img.shape)
            if uv is not None:
                p0 = X
                break

        if p0 is None:
            cv.putText(img, "Plane inliers do not project into view", (10, 60),
                       cv.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2, cv.LINE_AA)
            return img

        tmp = np.array([0.0, 0.0, 1.0], dtype=np.float64)  # camera forward
